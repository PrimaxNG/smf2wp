...
Function List: 
1. mike_conn($host,$user,$pass,$db) for DB Connection
2. br() for break line tag
3. fields($table)  => resource for table fields select query
4. field_num($result) => Total Number of fields in a table
5. value($array) => Display array list
6. value_arg() => displays list of an array argument (arry must be sent as arg)
7. field_name($ssql) => Get an arry list of all field nams in a table
8. insert_all($table_tab) => insert into a table
10. insert_selected($table_tab, $posted_var) => insert selected fileds
11. update_selected($table_tab, $posted_var, $where_field, $value) => update selected fields
12. update_all($table_tab, $where_field, $value) => update all fields in a db
13. delete($table_tab, $where_field, $value) => delete a db row
14. select_field($table, $name, $field, $known_value, $class) = > displays a list menu containing 
	db values
15. message($resource) => Display Successful or unsuccesful on resource query
16. while_sel($resource, $field_name, $symbol)=> display all data in a field
17. sel_query($table,$field,$where,$value)=> select query with where clause
18. counter($resource)=> return sthe number of rows fetched for query
19. currentpage() => returns page location
20. location($page, $ext, $url_var) => changes header location to choice page
21. fetch_assoc($resource) => returns a array for result resource
22. empty_url($url_var, $location) = > if empty url variable, redirect
23. select_field_where($table, $name, $field, $known_value, $class,$where,$value)=> combo box with where clause
24. select_field_where_empty($table, $name, $field, $known_value, $class,$where,$value)=> combo box with where clause without empty rows
25.  check_password($pass,$c_pass,$len)function to check for password validity and strlen
26. select_field_value($table, $name, $field, $known_value, $class) => box with values and labels different
27.  v_no_check($no,$sym) =>	//function to check vehicle number validity
28. today()=> dispalys todays date
29.  exist_b4($table,$field,$value)		 => check if a value exist in the db before
30.  explod($value,$sym)	 => explode a variab;e and return the array
31. function greater_time($dat)=> ##function to find if a date is graeter than present date
32. login($table,$id,$id_val,$pass,$pass_val,$st,$st_val)	=> verifies login parameters and returns number of valid validation
33. sel_query($table,$field,$where,$value,$orderval,$order)=> select statement with where and order values
34. sel_query_double($table,$field,$where,$value,$sec,$secval)=> double field check
35. date_convert($date,$type) => date from a format to any format

}
*/

/* connect to table to get field resourse
$ssql =  fields( config ) ; br();

//how many fields?
$num = field_num ($ssql ); 
echo $num; br();
// get array list of all table fields
$arrayfield = field_name( $ssql ); br();

//display fieldnames in array
$args = value_arg ( $arrayfield ); br();
*/
